...
def load_perspective_predictions(in_filepath):
    with open(in_filepath, 'r', encoding='utf-8') as in_file:
        ...
        preds = [float(json.loads(line)["logits_chosen"]) > float(json.loads(line)["logits_rejected"]) for line in in_file]
...
def calculate_agreement_proportions(filepaths):
    ...
    simplified_names = ["1st", "3rd", "prism"]
    return agreement_matrix, simplified_names

base = f"data/hh_labels/prism_35_{train_test}_"
filepaths = [
    base + "3_1.jsonl",
    base + "3_3.jsonl",
