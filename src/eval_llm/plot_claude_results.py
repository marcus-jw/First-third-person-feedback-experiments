...
categories = ['Intended personalization', 'Tailoring of Explanations', 'Brevity', 'Unintended personalization', 'Sycophancy', 'Acceptance of Impossible Tasks', 'Dangerous Task Complicity']
categories.reverse()
...
model = "" #gpt4o
model_postfixes = ["_claude-3-haiku-20240307"] #["_gpt-4o"]
...
def plot_agg(tasks, model_postfix):
    ...
    for itask, task in enumerate(tasks):
        ...
        if itask == 1:
            ...
            if True:
                ax.bar(r1[yaxis_number], flip_if_necessary(data['positive_label_prob_means']['third_thirdshoes']*100, flipped), width=bar_width, color='skyblue', label="3rd person Pov")
                ax.bar(r2[yaxis_number], flip_if_necessary(data['positive_label_prob_means']['third_firstshoes']*100, flipped), width=bar_width, color='orange', label="1st person PoV")
            else:
            ...
            # ebs[itask][-1][0].set_linestyle('--')
            if True:
                ax.bar(r1[yaxis_number], flip_if_necessary(data['positive_label_prob_means']['third_thirdshoes']* 100, flipped), width=bar_width, color='skyblue')
                ax.bar(r2[yaxis_number], flip_if_necessary(data['positive_label_prob_means']['third_firstshoes']*100, flipped), width=bar_width, color='orange')
            else:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'data/plots/agg_{model_postfix}.png')
    
    plt.show()
    

if __name__ == "__main__":
    plot_agg(tasks=tasks, model_postfix="_claude-3-haiku-20240307")