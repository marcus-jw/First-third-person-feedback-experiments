...
def aggregate_llm(model="gpt-4o", task="sycophancy"):
    ...
    with open(fname_in, "r", encoding="utf-8") as f_in, open(fname_out, "w", encoding="utf-8") as f_out:
        ...
        results = {
            ...
            "positive_label_prob_means": {
                key: np.mean(
                    [
                        row["positive_label_prob"][key]
                        / (row["positive_label_prob"][key] + row["negative_label_prob"][key])
                        for row in data
                    ]
                )
                for key in keys
            },
            "positive_label_prob_stds": {
                key: get_cl_margin_of_error([row["positive_label_prob"][key] for row in data]) for key in keys
            },
            "negative_label_prob_means": {
                key: np.mean(
                    [
                        row["negative_label_prob"][key]
                        / (row["positive_label_prob"][key] + row["negative_label_prob"][key])
                        for row in data
                    ]
                )
            ...
            "negative_label_prob_stds": {
                key: get_cl_margin_of_error([row["negative_label_prob"][key] for row in data]) for key in keys
            },
        }
...
if __name__ == "__main__":
    for task in ["danger_refusal", "impossible_task_refusal", "personalisation", "sycophancy", "verbosity"]:
        ...
        aggregate_llm(task=task, model="gpt-4o")
